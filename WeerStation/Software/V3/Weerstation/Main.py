...
class WeatherStationApp:
    def __init__(self):
        ...
        self.custom_font = ImageFont.truetype("font.ttf", font_size)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"Fout bij het verbinden met MySQL-database: {e}")
                print("Overschakelen naar SQLite-database...")
            ...
            try:
                ...
                print(f"Fout bij het invoegen van gegevens in MySQL-database: {e}")
                print("Overschakelen naar SQLite-database...")
                try:
                    self.db = WeatherDataDatabaseMySQL()
                    self.db.insert_data(self.sensor_data)
                    ...
                    switch_attempts += 1
                    print(f"Fout bij het invoegen van gegevens in SQLite-database: {e}")
                    print(f"Beide databases niet beschikbaar. Poging {switch_attempts}/{MAX_SWITCH_ATTEMPTS}")

                    if switch_attempts >= MAX_SWITCH_ATTEMPTS:
                        print("Maximaal aantal overstappogingen bereikt. Het programma wordt afgesloten.")
                        break
